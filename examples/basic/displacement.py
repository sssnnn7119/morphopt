    ...
    class Params(morphopt.Params):
        class GeometryParams(morphopt.GeometryParams):
            ...
            def apply_surface_constraints(self) -> None:
                ...
                """
                a: ThisController.Params.GeometryParams.BSP_NEW = self.surface_list[0]
                ...
                b: ThisController.Params.GeometryParams.BSP_NEW = self.surface_list[1]
                cp1 = b._cps.reshape(b.model.size[0], b.model.size[1], 3)
                cp1[:, :, 0] = (cp1[:, :, 0] + torch.flip(cp1[:, :, 0], dims=[1])) / 2
                cp1[:, :, 1] = (cp1[:, :, 1] - torch.flip(cp1[:, :, 1], dims=[1])) / 2
                cp1[:, :, 2] = (cp1[:, :, 2] + torch.flip(cp1[:, :, 2], dims=[1])) / 2
