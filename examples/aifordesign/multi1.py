...
class ThisController(morphopt.Controller):
    ...
    class ObjectiveFunction(morphopt.ObjectiveFunction):
        ...
        def pathlog_required(self):
            return super().pathlog_required() + ['inp&data']

        def objective_function(self):
            return (self.fe_results[0].GC[-6] - 50)**2 + (self.fe_results[0].GC[-4] - 20)**2
        ...
        def save(self, foldpath, iteration, insname = 'final_model', surface = 'surface_0_All'):
            ...
            src_inp = pathlib.Path(foldpath).parent / 'cache' / 'TopOptRun.inp'
            dst_dir = pathlib.Path(foldpath) / self.pathlog_required()[1]
            dst_dir.mkdir(parents=True, exist_ok=True)
            shutil.copy2(src_inp, dst_dir / f'multi1_{iteration}.inp')
            self.fe_results[0].save(dst_dir / f'multi1_{iteration}')
    ...
    class Params(morphopt.Params):
        class GeometryParams(morphopt.GeometryParams):
            ...
            def __init__(self):
                ...
                super().__init__(fea_seed_size=1.0, fea_mesh_order=1, reinitialize_per_iter=5)
...
if __name__ == '__main__':
    ...
    morphopt.start_optimization(device='cpu', no_gui=True)
