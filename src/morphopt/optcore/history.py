...
class History(ProtocalInitializable, ProtocalSavable):
    ...
    def append(self, name: str, value: any) -> None:
        ...
        if name not in self.data:
            ...
            self.data[name] = np.concatenate(
                (self.data[name], new_val.reshape(target_shape)), axis=0
            )
    ...
    def load(self, foldpath: str, iteration: int = None) -> None:
        ...
        for k, subs in key_subs.items():
            if k in ["time", "deformation"]:
                continue  # special arrays
            if k in scalar_keys:
                continue
        ...
        array_widths = {}
        for k, subs in key_subs.items():
            if k in scalar_keys or k == "deformation":
                continue
            max_idx = -1
            for s in subs:
                try:
            ...
            for k, v in row_data.items():
                if k in scalar_keys:
                    ...
                    if v:
                        max_idx = (
                            max(v.keys())
                            if all(isinstance(x, int) for x in v.keys())
                            else len(v) - 1
                        )
