...
class ClosedSurface(Surface_Base):
    ...
    def show(self,
        ...
        plotter = pv.Plotter()
        self._show(points3d, coo, color, alpha, plotter=plotter)
        plotter.show()
    ...
    def show_P0(self):
        ...
        mesh = pv.PolyData(control_points.T.numpy(), self.knots_element.numpy())
    ...
    def show_knots(self,
        ...
        mesh = pv.PolyData(knots.numpy(), elements.numpy())
        plotter.add_mesh(mesh, opacity=1)
        plotter.add_mesh(mesh, style='wireframe', color=(1.0/255, 1.0/255, 1.0/255), opacity=1)
        if not hold:
            plotter.show()
    ...
    def _show(self,
        ...
        r = self.map(points3d)
        faces = [[3] + list(face) for face in coo]
        mesh = pv.PolyData(r.T.numpy(), faces)
...
def show_surf2(r,
    ...
    r2 = torch.zeros_like(r[0])
    points_2d = torch.stack([r[0], r[1], r2], dim=0).T.numpy()
    faces = [[3] + list(face) for face in coo]
    mesh = pv.PolyData(points_2d, faces)
    plotter.add_mesh(mesh, opacity=1, color=color)
    plotter.add_mesh(mesh, style='wireframe', color=(1.0/255, 1.0/255, 1.0/255), opacity=1)
    if if_points:
        points_2d_points = torch.stack([r0[0], r0[1], torch.zeros_like(r0[0])], dim=0).T.numpy()
        plotter.add_points(points_2d_points, color=(40.0/255, 120.0/255, 181.0/255), point_size=4)
    if not hold:
        plotter.show()
...
def show_points(r, scale_factor=0.1):
    ...
    plotter = pv.Plotter()
    points = r.T.numpy()
    plotter.add_points(points, color=(1, 0, 0), point_size=scale_factor*100)  # pyvista point_size is in pixels
    plotter.show()
...
def show_points2(r, scale_factor=0.1):
    ...
    plotter = pv.Plotter()
    r2 = torch.zeros_like(r[0])
    points = torch.stack([r[0], r[1], r2], dim=0).T.numpy()
    plotter.add_points(points, color=(1, 0, 0), point_size=scale_factor*100)
    plotter.show()
