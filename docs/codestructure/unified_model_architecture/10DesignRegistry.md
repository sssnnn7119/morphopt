...
| 关联文档 | [总览](01-04Overview.md)、[FEA](07Fea.md)、[Updater](11-12Updaters.md)、[运行时](13-14Runtime.md) |
...
| `update_assembly(full_design_delta, categories=None)` | None | - | 切分试探增量，并按可选类别集合调用 owner 更新已绑定的 TorchFEA 对象 |
...
global design delta
    ...
    → Controller 建立逐工况 FEAController，Solver 完成平衡求解
    → SensitivityAnalyzer.build_sensitivities()
    → Updaters 计算 proposed changes
    → DesignRegistry.apply_design_delta(changes)
```

`update_assembly()` 修改 owner 已绑定的运行时引用并保留 autograd 图，owner 的已提交参数
保持不变。`categories=None` 表示更新全部类别；Controller 使用显式类别分两段更新：先把
geometry/material 写入共享基础 Assembly，再创建工况副本并绑定 `LoadValueBlock`，最后把
load 写入各自工况副本。每个 load owner 只接触一个 `(component_name, case_index)` 运行对象。
