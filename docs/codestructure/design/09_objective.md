...
`ObjectiveFunction` 接收共享 `FEAController` 和按 `step_index` 排序的 `StaticResult`，建立一次
...
| 项目 | 内容 |
|---|---|
| 输入 | 共享 `FEAController`、`StaticResult`、目标与指标代码、Jacobian 组件名称、设计变量注册表 |
| 输出 | 标量目标、逐工况指标、逐工况结果、结果网格、原生结果制品和按 `DesignKey` 切分的灵敏度 |
| 主要读者 | Objective、Controller、Updater、Codegen、Observer 和测试实现者 |
| 关联文档 | [FEA 组件](07_fea.md)、[Solver](08_solver.md)、[设计变量](10_design_registry.md)、[Updater](11_12_updaters.md)、[History](15_history.md) |
...
| `_fea_controller` | `torchfea.FEAController` 或 None | None | 当前结果对应的共享 TorchFEA 控制器 |
...
| `_mesh_cases` | dict[int, tuple[pyvista.DataSet, ...]] | `{}` | 已建立的逐工况结果网格 |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `build_evaluation()` | None | - | 使用当前共享控制器和结果建立目标及指标缓存 |
...
| `get_mesh_case(case_index)` | tuple[pyvista.DataSet, ...] | - | 读取指定工况已经建立的结果网格 |
...
| `compute_objective(case_assemblies, fe_results)` | torch.Tensor | - | 使用按工况索引提供的共享 Assembly 视图纯计算多工况总目标，供灵敏度分析复用 |
| `compute_sensitivities(registry, solver, fe_results)` | Mapping 或 None | - | 提供任务级灵敏度时返回各设计块结果；返回 `None` 时交给后端灵敏度流程 |
| `initialize(fea_params)` | None | `Initializable` | 校验组件引用、指标定义、工况权重和 Jacobian 请求 |
| `reinitialize(iteration, fea_controller, fe_results)` | None | `Initializable` | 绑定当前共享控制器与结果并清空上一轮评估和网格缓存 |
...
| `_build_result_mesh(assembly, result)` | tuple[pyvista.DataSet, ...] | 从对应工况 Assembly 和结果构造变形后网格、标量和向量数据 |
...
```text
objective.reinitialize(iteration, fea_controller, results)
...
共享 [UI 文档](16_17_ui_codegen.md) 定义的 Viewer 风格、色图和相机约定。
...
基座 Assembly 是几何与材料状态的共享载体，多工况 work condition 由库函数按结果自带的
...
2. registry.update_assembly(design_vars)（仅建立当前 Assembly 的灵敏度试探图）
...
| 阶段 | 图状态 |
|---|---|
| `Params.build_assembly()`、`build_case_assemblies()` | 全部在 detached 状态执行；只建立当前迭代的模型状态，不保留跨迭代计算图 |
| `SensitivityAnalyzer.build_sensitivities()` | 唯一把计算图延伸到 FEA 的阶段：全局设计叶子 → 共享 Assembly → 多工况 work condition 与因子分解 → `StaticResult.GC`/`jacobian` 梯度叶子 → 多工况目标 → 伴随求解 |
