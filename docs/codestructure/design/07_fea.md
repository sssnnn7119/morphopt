...
FEAParams.build_case_assemblies()
    → 在共享 Assembly 上准备全部 LoadStep 的载荷值
    → 将 LoadValueBlock 绑定到共享 component 运行对象
    → LoadValueBlock.update_assembly(design_delta)
    → Controller.build_fea_controller()
```
...
| 关联文档 | [几何系统](05_geometry.md)、[材料系统](06_materials.md)、[Solver](08_solver.md)、[目标函数](09_objective.md)、[运行时](13_14_runtime.md) |
...
| `_component_targets` | dict[str, torchfea.Serializable] | `{}` | component 到 Assembly 目标的索引 |
| `_case_assemblies` | tuple[torchfea.Assembly, ...] | `()` | 按工况索引返回同一个共享 Assembly 的视图 |
| `_torchfea_FEAController` | torchfea.FEAController 或 None | None | 当前 Assembly 上承载全部工况的共享控制器 |
...
| `build_case_assemblies()` | None | - | 在共享 Assembly 上准备全部 LoadStep 值和 case 索引视图 |
| `get_case_assemblies()` | tuple[`torchfea.Assembly`, ...] | - | 按工况读取共享 Assembly 引用 |
| `update_fea(step_index)` | None | - | 将指定工况的载荷值写入共享控制器的 LoadStep 上下文 |
...
| `get_meshes()` | list[pyvista.DataSet] | `Visualizable` | 读取全部 component 预览 |
...
| `_build_case_values(step_index)` | None | 将指定 LoadStep 的值写入共享 component 上下文 |
...
component 目标。Assembly 保存当前迭代的共享几何、材料和 component 对象；每个工况只保存
不同的 LoadStep 数值和索引。工况建立顺序固定为“写入 LoadStep 值 → 绑定 LoadValueBlock →
由 Controller 将全部工况交给一个 FEAController”。FEAParams 只负责组件、工况和 Assembly
上的运行对象，控制器生命周期由 `Controller.build_fea_controller()` 统一管理。
...
| 类别 | 内容 |
|---|---|
| 共享对象 | 一个 `Assembly`、全部 `Part`/Instance、几何与材料 Tensor、ReferencePoint、FEA component 运行对象和一个 `FEAController` |
| 工况差异 | `LoadStep` 的值向量、载荷 Jacobian 名称和结果的 `step_index` |
| 更新规则 | `LoadValueBlock` 只更新共享控制器当前的工况值上下文，不复制 Assembly 或 component |
| 求解规则 | 一个 `FEAController` 一次接收全部 LoadStep，统一完成平衡、Jacobian 和多工况灵敏度 |
| 内存 | 工况数只增加值向量和结果缓存，避免按工况复制完整模型 |
...
| `_target` | torchfea.Serializable 或 None | None | 从当前 Assembly 解析出的目标对象 |
| `_mesh_cache` | list[pyvista.DataSet] | `[]` | 已建立的 component 预览网格 |
...
| `get_fea_object()` | torchfea.Serializable | - | 读取已经创建的 TorchFEA 对象 |
...
| `get_meshes()` | list[pyvista.DataSet] | `Visualizable` | 读取已经建立的预览缓存 |
...
| 方法 | 返回值 | 作用 |
|---|---|---|
| `_resolve_target(assembly)` | torchfea.Serializable | 根据目标名称解析 Assembly 对象 |
| `_validate_values(values)` | None | 校验值向量长度和数值类型 |
| `_create_fea_object()` | torchfea.Serializable | 由具体类型创建 TorchFEA 对象 |
| `_apply_values(values)` | None | 由具体类型把已校验值写入缓存的 TorchFEA 对象 |
| `_build_preview_meshes()` | list[pyvista.DataSet] | 由具体类型建立预览几何，供 `build_meshes()` 写入缓存 |

`build_fea()` 只创建运行时对象并写入具体子类的 `_torchfea_<ConcreteName>`；
`get_fea_object()` 读取已建立对象，`assign_fea()` 完成 Assembly 挂接，
`update_fea()` 更新当前工作条件。`FEAParams._build_case()` 刷新共享 component
的当前工况值，并在共享 Assembly 上维护统一的 TorchFEA component。
逐工况参数由 `LoadStep` 保存；`LoadValueBlock` 把一个
`(component_name, case_index)` 组合适配为 `Updatable` owner。
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `_resolve_target(assembly)` | torchfea.Serializable | `BaseFEAComponent` | 解析目标 Surface set |
| `_create_fea_object()` | torchfea.loads.Pressure | `BaseFEAComponent` | 创建 TorchFEA Pressure |
| `_apply_values(values)` | None | `BaseFEAComponent` | 更新压力标量 |
| `_build_preview_meshes()` | list[pyvista.DataSet] | `BaseFEAComponent` | 建立曲面法向压力箭头 |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `_resolve_target(assembly)` | torchfea.Serializable | `BaseFEAComponent` | 解析目标元素集合 |
| `_create_fea_object()` | torchfea.loads.BodyForce | `BaseFEAComponent` | 创建 TorchFEA BodyForce |
| `_apply_values(values)` | None | `BaseFEAComponent` | 更新力密度及 TorchFEA 积分缓存 |
| `_build_preview_meshes()` | list[pyvista.DataSet] | `BaseFEAComponent` | 在元素集合中心建立体力箭头 |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `_resolve_target(assembly)` | torchfea.ReferencePoint | `BaseFEAComponent` | 解析目标参考点 |
| `_create_fea_object()` | torchfea.loads.Concentrate_Force | `BaseFEAComponent` | 创建 TorchFEA ConcentratedForce |
| `_apply_values(values)` | None | `BaseFEAComponent` | 更新三方向集中力 |
| `_build_preview_meshes()` | list[pyvista.DataSet] | `BaseFEAComponent` | 建立参考点处的力箭头 |
...
|---|---|---|---|
| `_resolve_target(assembly)` | torchfea.ReferencePoint | `BaseFEAComponent` | 解析目标参考点 |
| `_create_fea_object()` | torchfea.loads.Moment | `BaseFEAComponent` | 创建 TorchFEA ConcentratedMoment |
| `_apply_values(values)` | None | `BaseFEAComponent` | 更新三方向集中力矩 |
| `_build_preview_meshes()` | list[pyvista.DataSet] | `BaseFEAComponent` | 建立参考点处的力矩圆弧与方向箭头 |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `_resolve_target(assembly)` | torchfea.Serializable | `BaseFEAComponent` | 解析目标 Node set 并校验自由度 |
| `_create_fea_object()` | torchfea.boundarys.Boundary_Condition | `BaseFEAComponent` | 创建 TorchFEA BoundaryCondition |
| `_apply_values(values)` | None | `BaseFEAComponent` | 校验零长度值向量并保持边界状态 |
| `_build_preview_meshes()` | list[pyvista.DataSet] | `BaseFEAComponent` | 建立受约束节点和自由度方向标记 |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `_resolve_target(assembly)` | torchfea.ReferencePoint | `BaseFEAComponent` | 解析目标参考点并校验自由度 |
| `_create_fea_object()` | torchfea.boundarys.Boundary_Condition_RP | `BaseFEAComponent` | 创建 TorchFEA BoundaryConditionRP |
| `_apply_values(values)` | None | `BaseFEAComponent` | 校验零长度值向量并保持边界状态 |
| `_build_preview_meshes()` | list[pyvista.DataSet] | `BaseFEAComponent` | 建立参考点和受约束自由度标记 |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `_resolve_target(assembly)` | tuple[torchfea.ReferencePoint, torchfea.Serializable] | `BaseFEAComponent` | 解析 RP 和 Node set |
| `_create_fea_object()` | torchfea.constraints.Couple | `BaseFEAComponent` | 创建 TorchFEA Couple |
| `_apply_values(values)` | None | `BaseFEAComponent` | 校验零长度值向量并保持耦合状态 |
| `_build_preview_meshes()` | list[pyvista.DataSet] | `BaseFEAComponent` | 建立 RP 到耦合节点集合的连线 |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `_resolve_target(assembly)` | torchfea.ReferencePoint | `BaseFEAComponent` | 解析参考点 |
| `_create_fea_object()` | torchfea.loads.Spring_RP_Point | `BaseFEAComponent` | 创建 TorchFEA SpringToGround |
| `_apply_values(values)` | None | `BaseFEAComponent` | 更新刚度、原长和固定点 |
| `_build_preview_meshes()` | list[pyvista.DataSet] | `BaseFEAComponent` | 建立参考点到固定点的弹簧线 |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `_resolve_target(assembly)` | tuple[torchfea.ReferencePoint, torchfea.ReferencePoint] | `BaseFEAComponent` | 解析两个参考点 |
| `_create_fea_object()` | torchfea.loads.Spring_RP_RP | `BaseFEAComponent` | 创建 TorchFEA SpringBetweenRPs |
| `_apply_values(values)` | None | `BaseFEAComponent` | 更新刚度和原长 |
| `_build_preview_meshes()` | list[pyvista.DataSet] | `BaseFEAComponent` | 建立两个参考点之间的弹簧线 |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `_resolve_target(assembly)` | torchfea.Serializable | `BaseFEAComponent` | 根据对象名称和类型解析目标自由度 |
| `_create_fea_object()` | torchfea.loads.Penalty_DoF | `BaseFEAComponent` | 创建 TorchFEA PenaltyDoF |
| `_apply_values(values)` | None | `BaseFEAComponent` | 更新惩罚系数和目标值 |
| `_build_preview_meshes()` | list[pyvista.DataSet] | `BaseFEAComponent` | 建立目标对象与自由度方向标记 |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `_resolve_target(assembly)` | tuple[torchfea.Serializable, torchfea.Serializable] | `BaseFEAComponent` | 解析两个 Surface |
| `_create_fea_object()` | torchfea.loads.Contact | `BaseFEAComponent` | 创建 TorchFEA Contact |
| `_apply_values(values)` | None | `BaseFEAComponent` | 校验零长度值向量并保持接触参数 |
| `_build_preview_meshes()` | list[pyvista.DataSet] | `BaseFEAComponent` | 建立两侧接触面的分组预览 |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `_resolve_target(assembly)` | torchfea.Serializable | `BaseFEAComponent` | 解析目标 Surface |
| `_create_fea_object()` | torchfea.loads.ContactSelf | `BaseFEAComponent` | 创建 TorchFEA SelfContact |
| `_apply_values(values)` | None | `BaseFEAComponent` | 校验零长度值向量并保持接触参数 |
| `_build_preview_meshes()` | list[pyvista.DataSet] | `BaseFEAComponent` | 建立自接触曲面的高亮预览 |
...
并将该值写入共享 Assembly 的 component 运行对象。
...
| `_torchfea_Component` | `torchfea.loads.*` 或 None | None | 共享 Assembly 中接收当前工况值的 component 运行对象 |
| `_torchfea_Assembly` | torchfea.Assembly 或 None | None | 所有工况共用的 Assembly |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `update_case_binding(assembly, component)` | None | - | 绑定共享 Assembly 和已经挂接的 component 运行对象 |
...
| `update_assembly(design_delta)` | None | `Updatable` | 将已提交值与试探增量之和写入共享 component 的当前工况值 |
...
`FEAParams.build_case_assemblies()` 刷新共享 Assembly 的工作条件，再调用
`update_case_binding()`。随后 Registry 只把该 block 的局部设计增量写入共享 component 的
当前工况值。`LoadStep` 始终保存已提交值，试探值由工作条件上下文维护，因此 line search、
梯度检查和多个工况的灵敏度计算可以同时保留各自的 autograd 图。
