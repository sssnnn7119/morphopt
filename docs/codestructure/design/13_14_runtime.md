...
[`History`](15_history.md) 文档定义。返回[总入口](../design.md)。
...
| 关联文档 | [总览](01_04_overview.md)、[Solver](08_solver.md)、[目标函数](09_objective.md)、[Updater](11_12_updaters.md)、[History](15_history.md) |
...
| `get_meshes()` | tuple[pyvista.DataSet, ...] | `Visualizable` | 读取已经建立的聚合预览 |
...
| `_device` | str | `"cpu"` | Controller 编排和辅助 Tensor 使用的设备 |
| `_solver_device_names` | tuple[str, ...] | `()` | 独立的 FEA Solver 设备列表 |
| `_updater_device` | str | `"cpu"` | 独立的 Updater 局部优化设备 |
...
| `_data_queue` | multiprocessing.Queue[RuntimeEvent] 或 None | None | 向 UI/父进程发送结构化进度的队列 |
| `_stop_event` | multiprocessing.Event 或 None | None | 任务运行器提供的跨进程停止事件 |
...
| `_torchfea_FEAController` | `torchfea.FEAController` 或 None | None | 当前 Assembly 上承载全部工况的共享 TorchFEA 控制器；测试使用其轻量子类 |
...
| `device` | str | 只读 | 内部维护 | 返回 Controller 编排设备 |
| `solver_device_names` | tuple[str, ...] | 只读 | 内部维护 | 返回 FEA Solver 设备配置 |
| `updater_device` | str | 只读 | 内部维护 | 返回 Updater 局部优化设备 |
...
| `data_queue` | multiprocessing.Queue[RuntimeEvent] 或 None | 只读 | 内部维护 | 返回进度队列 |
| `stop_event` | multiprocessing.Event 或 None | 只读 | 内部维护 | 返回跨进程停止事件 |
...
| `build_fea_controller()` | None | - | 用共享 Assembly 创建承载全部工况的 `FEAController` |
| `get_fea_controller()` | `torchfea.FEAController` 或 None | - | 读取已经建立的共享求解控制器 |
...
| `change_device(device)` | None | - | 更新 Controller 编排设备 |
| `change_solver_devices(device_names)` | None | - | 更新后续 FEA Solver 设备配置 |
| `initialize()` | None | `Initializable` | 创建并连接 Params、Solver、Objective、SensitivityAnalyzer、Registry、Updaters 和 History |
| `save(folder_path, iteration)` | None | `Persistable` | 原子保存模型恢复状态和 checkpoint manifest；历史写入运行目录 `logs/` |
| `load(folder_path, iteration)` | None | `Persistable` | 按 manifest 恢复模型状态；历史由运行目录 `logs/` 单独解析 |
...
10. 标记初始化完成；进程创建、worker 重启和资源回收由 `TaskRunner` 负责。
```
...
4. fea.build_case_assemblies()  # 将新 Assembly 绑定到全部 LoadStep 工况
5. registry.reinitialize(iteration)
6. registry.build_design_delta()
7. build_fea_controller()；创建一个承载全部工况的 FEAController
8. solver.reinitialize(iteration)
9. solver.build_solvers(fea_controller) 并设置 FEAController.solver
10. solver.solve(fea_controller, objective.jacobian_needed)
11. results = solver.get_results()
12. objective.reinitialize(iteration, fea_controller, results)
13. objective.build_evaluation()
14. sensitivity.reinitialize(iteration, results)
15. sensitivity.build_sensitivities()；内部以 registry.update_assembly() 建立试探图
...
20. registry.apply_design_delta(changes)；提交到 Params 的设计定义
21. 返回 StepResult
```

本表是一次外层迭代顺序的唯一权威来源：[Solver](08_solver.md) 的调用示例、
[设计变量注册](10_design_registry.md) 的试探-提交流程和 [总览](01_04_overview.md) 的迭代图
按同一顺序执行。`solver.reinitialize(iteration)` 位于 `solver.build_solvers()` 之前，
初值策略和 GC 缓存在 `solver.build_solvers()` 前准备；`build_fea_controller()` 创建
共享 `FEAController` 并设置 assembly，求解器挂接由 `solver.build_solvers()` 完成。

本轮 Assembly 的构建只读取 Params 中已经提交的定义。灵敏度阶段由
`SensitivityAnalyzer` 调用 `DesignRegistry.update_assembly()` 建立当前 Assembly 的
试探计算图；Updater 完成提交后，下一轮 `Params.build_assembly()` 读取新的定义并创建
新的 Assembly。

`_opt_loop()` 对每个成功的 `StepResult` 先导出本轮制品，再建立并追加 `HistoryRecord`，将
历史快照写入运行目录 `logs/`，按 `checkpoint_interval` 保存模型 checkpoint，发布进度事件并判断停止条件。标准模式累计完成
...
restart_optimization(result_path, target_iteration)
    → 绑定已有结果目录
    → initialize()
    → History.load(run_path/logs, target_iteration)
    → load(checkpoint, target_iteration)  # 仅模型恢复状态
    → _opt_loop(start_iteration=history.get_current_iteration() + 1)
```

运行入口捕获异常并发布失败事件；当前 Controller 不持有 worker 池，进程创建、重启和资源
回收由 `TaskRunner` 统一负责。debug 模式在当前进程执行任务；标准模式使用 spawn 子进程并通过结构化队列报告
...
| `_payload` | Mapping[str, JsonValue] | `{}` | 与事件类型匹配的可序列化载荷 |
...
| `payload` | Mapping[str, JsonValue] | 只读 | 内部维护 | 返回载荷只读视图 |

### 外部接口方法

| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `to_dict()` | JsonObject | - | 生成可跨进程传输的事件数据 |
...
| `_event_queue` | multiprocessing.Queue[RuntimeEvent] 或 None | None | 可选结构化事件队列 |
...
| 函数 | 返回值 | 作用 |
|---|---|---|
| `_run_task()` | None | 创建 Controller，选择新建/重启入口并把结果路径写入共享状态 |
| `_run_supervisor()` | None | 循环启动任务子进程；收到 restart payload 后读取 checkpoint 并启动下一段 |
| `_publish_failure(error)` | None | 发送结构化失败事件并写入 traceback 日志 |
| `_configure_child_environment()` | None | 设置线程数、日志、设备可见性和工作目录 |

退出码约定为：`0` 正常完成，`2` 用户停止，`1` 定义、初始化、求解或保存失败。
达到 worker 回收轮次时，Controller 先写入完整 checkpoint 并发布 `restart_requested` 事件，
当前 worker 正常退出；监督器根据事件携带的结果目录和 iteration 启动下一段子进程。
debug 模式在当前进程连续运行并保留原始 traceback。
...
包含恢复计算所需的模型状态：`Params`（含 `GeometryParams`、`MaterialsParams`、
`FEAParams` 与已提交的 `LoadStep` 值）、`Solver`（配置、工况分组、可复用 GC）、
`DesignRegistry`（key、区间、形状与已提交参数）、`Updaters`（优化器记忆与步长）和
`ObjectiveFunction`（Jacobian 请求与指标）。`History` 不属于 checkpoint；它的唯一持久化位置是
运行目录的 `logs/history.json` 与 `logs/history.csv`。
...
| `restart_requested` 事件 | 已写入完整 checkpoint，请求监督器续跑 | 读取事件中的 checkpoint，在同一结果目录启动下一段子进程 |

`Controller` 每完成一轮 `step()` 时先更新 `logs/` 历史，再按
`iteration % checkpoint_interval == 0` 写入模型 checkpoint；当连续完成的迭代数达到
`worker_restart_interval`（且该值大于 `0`）时，`Controller` 写入完整模型 checkpoint 并发布
`restart_requested` 事件后结束当前 worker。
`TaskRunner._run_supervisor()` 只根据事件队列中的 restart payload 续跑。
...
| 子进程在写 checkpoint 中途退出 | 该次 checkpoint 没有 manifest，视为不存在；模型恢复时回退到最近一个完整 checkpoint，历史仍从 `logs/` 解析 |
| `checkpoint_interval > 1` | 恢复时按 14.7.1 的完整性规则向前查找最近一个完整 checkpoint |
| 续跑的起始迭代 | `History.load(logs)` 恢复后从 `history.get_current_iteration() + 1` 继续；同一迭代不会重复求解 |
...
`History.add_record()` 与日志写入在同一轮内完成：先写制品，再追加记录，最后原子更新
`logs/history.json` 与 `logs/history.csv`；模型 checkpoint 随后独立写入。因此历史记录和
模型状态的恢复来源清晰分离：历史以日志为准，模型以最近完整 checkpoint 为准。
