...
本文件定义 UI 数据树、编辑器字段、任务源码生成和运行结果目录。返回[总入口](../design.md)。
...
本文件使用的 `JsonValue`/`JsonObject` 来自 `optcore.protocols`；`NodeSchema`、
`ConstraintSchema`、`ConsoleOperation`、`Actor` 和 `QTreeWidgetItem` 分别是 UI 模型、
控制台解析器、可视化器和 Qt 层定义的具体数据类。字段表不使用未定义的通用类型。
...
| 关联文档 | [总览与生命周期](01_04_overview.md)、[几何系统](05_geometry.md)、[运行时](13_14_runtime.md)、[History](15_history.md)、[迁移验收](18_22_validation_migration.md) |
...
| 属性 | 类型 | 初始值 | 说明 |
|---|---|---|---|
| `_node_index` | dict[str, `DefinitionNode`] | {} | 节点名称索引 |
...
| `to_dict()` | JsonObject | - | 生成带 schema 版本的纯数据定义 |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `get_field(field_name)` | JsonValue | - | 读取已经存在的可编辑字段 |
...
| `generate_data()` | Mapping[str, JsonValue] | - | 生成持久化和 Codegen 使用的纯数据 |

#### 内部辅助函数

| 函数 | 返回值 | 作用 |
|---|---|---|
| `_coerce_field(name, value)` | JsonValue | 按节点 schema 转换单个字段 |
...
| `_fields` | dict[str, JsonValue] | `{}` | 与 Part 类型匹配的构造参数 |
...
| `fields` | Mapping[str, JsonValue] | - | 只读 | 内部维护 | 返回构造参数只读视图 |
...
| `_fields` | dict[str, JsonValue] | `{}` | 曲面构造参数和来源 |
...
| `fields` | Mapping[str, JsonValue] | - | 只读 | 内部维护 | 返回曲面参数只读视图 |
...
| `_fields` | dict[str, JsonValue] | `{}` | 本构和材料场参数 |
...
| `fields` | Mapping[str, JsonValue] | - | 只读 | 内部维护 | 返回材料参数只读视图 |
...
| `_fields` | dict[str, JsonValue] | `{}` | 组件专用参数 |
...
| `fields` | Mapping[str, JsonValue] | - | 只读 | 内部维护 | 返回专用参数只读视图 |
...
| `_optimizer_options` | dict[str, JsonValue] | `{}` | 局部优化器配置 |
| `_local_constraints` | list[JsonObject] | `[]` | updater 专属约束定义 |
| `_regularization_terms` | list[JsonObject] | `[]` | 材料正则项 |
...
| `optimizer_options` | Mapping[str, JsonValue] | - | 只读 | 读写 | 返回或替换优化器配置 |
| `local_constraints`、`regularization_terms` | tuple[JsonObject, ...] | - | 只读 | 内部维护 | 返回局部项定义 |
...
| 属性 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `_fields` | dict[str, JsonValue] | 求解器默认值 | maximum iterations、tolerance、process、devices、task groups 和初值复用配置 |
...
| property | 类型 | 来源 | 读权限 | 写权限 | 说明 |
|---|---|---|---|---|---|
| `fields` | Mapping[str, JsonValue] | - | 只读 | 内部维护 | 返回求解器配置只读视图 |
...
| 属性 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `_schema_registry` | Mapping[str, `NodeSchema`] | - | 节点类型到字段 schema 的只读注册表 |

#### 运行时属性（`__init__()` 声明，校验时填充）

| 属性 | 类型 | 初始值 | 说明 |
|---|---|---|---|
| `_issues` | list[JsonObject] | `[]` | 最近一次按节点路径排序的校验问题 |

#### 属性接口（property）

| property | 类型 | 来源 | 读权限 | 写权限 | 说明 |
|---|---|---|---|---|---|
| `schema_registry` | Mapping[str, `NodeSchema`] | - | 只读 | 内部维护 | 返回字段 schema 只读视图 |
...
| `get_issues()` | tuple[JsonObject, ...] | - | 读取最近一次校验的问题代码、节点路径、字段和双语消息 key |
...
| 属性 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `_schema_registry` | Mapping[str, `NodeSchema`] | - | 节点字段、控件类型、双语标签和动态选项定义 |
...
| `_get_dynamic_choices(field_name)` | tuple[str, ...] | 从当前问题和模型摘要读取名称选择项 |
...
| `_preview_meshes` | tuple[pyvista.DataSet, ...] | `()` | 隔离检查进程返回的预览网格 |
...
| `get_preview_meshes()` | tuple[pyvista.DataSet, ...] | - | 读取已经建立的模型预览 |
...
3. 选择 [`HomogeneousMaterial`](06_materials.md#65-homogeneousmaterial) 或
   [`SIMPFieldMaterial`](06_materials.md#66-simpfieldmaterial)；
4. 选择参数类并填写全部参数；
5. 填写接口类型的附加字段。

材料节点最终注册到 [`MaterialsParams`](06_materials.md#61-materialsparams)，并通过
...
材料场和 `VolFrac` 只在 [`SIMPFieldMaterial`](06_materials.md#66-simpfieldmaterial) 页面显示。

### 16.4 FEA 编辑器

FEA 编辑器中的类顺序与
[FEA 组件](07_fea.md)保持一致：
[`FEAParams`](07_fea.md#71-feaparams) →
[`BaseFEAComponent`](07_fea.md#72-basefeacomponent) → FEA component 子类 →
[`LoadStep`](07_fea.md#715-loadstep)。Solver 使用独立的 [08_solver.md](08_solver.md) 接口。

| 编辑内容 | 目标选择 |
|---|---|
| [`Pressure`](07_fea.md#73-pressure) | `Instance` + surface set |
| [`BodyForce`](07_fea.md#74-bodyforce) | `Instance` + `element_name` |
| [`ConcentratedForce`](07_fea.md#75-concentratedforce) | `ReferencePoint` |
| [`ConcentratedMoment`](07_fea.md#76-concentratedmoment) | `ReferencePoint` |
| [`BoundaryCondition`](07_fea.md#77-boundarycondition) | `Instance` + node set |
| [`BoundaryConditionRP`](07_fea.md#78-boundaryconditionrp) | `ReferencePoint` |
| [`Couple`](07_fea.md#79-couple) | `Instance` + `ReferencePoint` |
| [`SpringToGround`](07_fea.md#710-springtoground) | `ReferencePoint` + DOF |
| [`SpringBetweenRPs`](07_fea.md#711-springbetweenrps) | 两个 `ReferencePoint` + DOF |
| [`PenaltyDoF`](07_fea.md#712-penaltydof) | `Instance`/节点 + DOF |
| [`Contact`](07_fea.md#713-contact) | 两个 `Instance` + surface set |
| [`SelfContact`](07_fea.md#714-selfcontact) | 一个 `Instance` + surface set |
| [`LoadStep`](07_fea.md#715-loadstep) | FEA component 名称和 step 值 |

这些对象由 [`FEAParams`](07_fea.md#71-feaparams) 先执行
...
| 属性 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `_constraint_schemas` | Mapping[str, `ConstraintSchema`] | - | 各 updater 可用约束、参数和双语标签 |
...
| property | 类型 | 来源 | 读权限 | 写权限 | 说明 |
|---|---|---|---|---|---|
| `constraint_schemas` | Mapping[str, `ConstraintSchema`] | - | 只读 | 内部维护 | 返回约束 schema 只读视图 |
| `snippet_catalog` | `SnippetCatalog` | - | 只读 | 内部维护 | 返回代码片段目录 |
...
| `get_preview_meshes()` | tuple[pyvista.DataSet, ...] | - | 读取已完成预览网格 |
...
| `_item_to_node` | dict[QTreeWidgetItem, `DefinitionNode`] | `{}` | Qt item 到定义节点的映射 |
...
| `_build_context_menu(node)` | QMenu | 根据节点能力建立增删、复制、移动和重命名动作 |
...
| 属性 | 类型 | 初始值 | 说明 |
|---|---|---|---|
| `_context` | Mapping[str, JsonValue] | `{}` | 当前工况、模型名称、结果字段和类型信息 |
...
| `default` | JsonValue | - | 只读 | 内部维护 | 返回默认值 |
...
| `_renderer` | Callable[[Mapping[str, JsonValue]], str] | - | 英文 Python 代码渲染函数 |
...
| `_context` | Mapping[str, JsonValue] | - | 动态名称和模型摘要上下文 |
...
| `context` | Mapping[str, JsonValue] | - | 只读 | 内部维护 | 返回动态上下文只读视图 |

###### 外部接口方法

| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `get_parameter_values()` | Mapping[str, JsonValue] | - | 读取当前控件中的规范化参数 |
...
| `_style` | Mapping[str, str | int] | `{}` | 当前 SGR 文本样式 |
...
| 方法 | 返回值 | 来源 | 作用 |
|---|---|---|---|
| `feed(text)` | tuple[`ConsoleOperation`, ...] | - | 消费一个文本 chunk 并返回追加、替换或删除行操作 |
| `flush()` | tuple[`ConsoleOperation`, ...] | - | 将结尾普通文本输出并保留完整终端状态 |
...
| 函数 | 返回值 | 作用 |
|---|---|---|
| `_consume_escape(sequence)` | list[`ConsoleOperation`] | 解析 SGR、CSI 光标和清除指令 |
| `_write_text(text)` | list[`ConsoleOperation`] | 按当前光标处理换行、回车和 backspace |
...
| 属性 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `_theme` | Mapping[str, str | float | bool] | 默认深色主题 | 背景、实体、边线、选中和标量色图配置 |
...
| `_actors` | dict[str, `Actor`] | `{}` | 稳定 mesh id 到 actor 的映射 |
| `_meshes` | tuple[pyvista.DataSet, ...] | `()` | 当前显示网格 |
...
| property | 类型 | 来源 | 读权限 | 写权限 | 说明 |
|---|---|---|---|---|---|
| `theme` | Mapping[str, str | float | bool] | - | 只读 | 内部维护 | 返回统一显示主题只读视图 |
...
| `get_meshes()` | tuple[pyvista.DataSet, ...] | - | 读取当前显示网格 |
...
| 函数 | 返回值 | 作用 |
|---|---|---|
| `_build_actor(mesh, options)` | `Actor` | 使用统一材质、光照和边线策略建立 actor |
...
| `refresh()` | None | - | 增量解析运行目录 `logs/` 的新历史记录并刷新结果索引 |

##### 内部辅助函数

| 函数 | 返回值 | 作用 |
|---|---|---|
| `_load_checkpoint(iteration)` | None | 加载该轮模型状态、结果和可视化缓存；历史由 `logs/` 单独读取 |
...
| 属性 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `_formatter_options` | Mapping[str, JsonValue] | `{}` | 导入分组、缩进和稳定排序配置 |
...
| property | 类型 | 来源 | 读权限 | 写权限 | 说明 |
|---|---|---|---|---|---|
| `formatter_options` | Mapping[str, JsonValue] | - | 只读 | 内部维护 | 返回格式配置只读视图 |
...
归入对应 updater 目录，见[破坏性变更](24_breaking_changes.md)。通用
...
| `ObjectiveFunction` | 目标值、Jacobian 请求和指标配置 | 指标配置与目标运行状态 |
| `History` | 运行目录 `logs/history.json`、`logs/history.csv` | 解析运行目录 `logs/` 中的历史记录和结果索引 |
...
| 继续计算 | 运行目录或结果目录 | 由 `load_controller()` 解析 `logs/` 历史并读取最新完整模型 checkpoint，显示其迭代号，用户选择目标迭代（默认最新完整迭代）后运行 |

规则：

- 三个入口都通过 `TaskLauncher` 启动独立进程，UI 进程不 import 任务模块；
- 运行期间不允许编辑定义树；停止通过 `TaskLauncher.stop()` 发送终止信号，对应退出码 `2`；
- 继续计算不以编辑树为准：它以 checkpoint 的任务文件与签名为唯一权威输入，历史以运行目录 `logs/` 为唯一来源，签名不一致时
  报告错误并终止，不回退到编辑树内容；
