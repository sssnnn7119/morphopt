...
本文件定义全局目标、命名、属性权限、协议、生命周期和文档维护规范。返回[总入口](../design.md)。
...
| 关联文档 | [编码规范](../style.md)、[几何系统](05_geometry.md)、[材料系统](06_materials.md)、[运行时](13_14_runtime.md)、[History](15_history.md) |
...
FEAParams 创建 component 并写入同一个 Assembly。`Controller` 用共享 Assembly 创建一个
`torchfea.FEAController`；Solver 在同一个控制器中挂接 `StaticImplicitSolver`，随后一次完成全部工况求解。
...
包含三大 Params，三大 Params 共同加工一个 Assembly，Controller 组合一个共享
`FEAController`，Solver 在同一控制器中执行多工况求解与灵敏度分析。
...
接口缺失应立即抛出错误。后端能力确实可选时，在对应适配边界使用明确的能力分支并记录
缺失能力的处理方式。
...
| `build_solvers(fea_controller)` | 由 `Solver` 根据求解配置为共享 `FEAController` 创建并挂接 `StaticImplicitSolver` | 方法返回 `None`；结果通过 `get_solver()` 读取 |
...
| 求解 | `Solver` | 共享多工况静力求解器创建、挂接与多进程 FEA 求解 |
...
FEAParams
    ...
    → assign_components()
    → build_case_assemblies()  # 刷新共享 Assembly 上各 LoadStep 的载荷值
    → DesignRegistry.update_assembly(delta, {"load"})
    → get_case_assemblies()    # 返回同一 Assembly 的工况视图

Solver
    → build_solvers(fea_controller)
    → solve(fea_controller)
    → get_results()
~~~
...
geometry/material 试探值写入共享 Assembly；`FEAParams.build_case_assemblies()` 再保存每个
工况不同的 LoadStep/LoadValueBlock 数值，共享 Assembly 和 component 运行对象；Registry
将 load 试探值写入共享控制器的工况值数组。Controller 创建一个共享 `FEAController`，Solver
在其中一次完成多工况平衡、Jacobian 和灵敏度计算。
...
| `morphopt/task.py` | `TaskRunner`、`start_optimization()` | 任务进程、退出码与续跑入口，见[运行时](13_14_runtime.md) |
| `morphopt/_torch.py` | 可选 Torch 适配层 | 正式运行使用 PyTorch；基础流程测试提供轻量 NumPy fallback |
| `morphopt/optcore/` | 运行时核心 | `controller.py`、`protocols.py`、`design_registry.py`、`objective.py`、`sensitivity.py`、`solver.py`、`history.py` 与 `modelparams/` |
| `morphopt/optcore/updaters/` | 优化核心中的 updater 与优化算法 | `base.py`、`optimizers.py`、`geometry.py`、`material.py`、`fea.py`、`terms.py` |
| `morphopt/utils/` | 工具函数 | `gradient_check.py`、`history_read.py` |
| `morphopt/ui/` | 图形界面 | 只生成与观察任务，不创建运行时对象 |

不存在模块级全局 `controller`：所有对象通过构造函数或 `initialize()` 注入，`Controller`
持有全部运行时对象并由任务入口创建。V3 被移除的入口清单见
[破坏性变更与冻结契约](24_breaking_changes.md)。
...
| `FEAParams` | FEA component 和 load step 定义 | 一个共享 `Assembly`、LoadStep 载荷值和多工况控制器配置 |
...
| `Solver` | 进程、设备、任务组和求解选项 | 一个共享 TorchFEA 求解器上下文及多工况结果 |
| `ObjectiveFunction` | 指标名称、工况权重和 Jacobian 名称 | 静态引用校验；每轮共享 `FEAController` 和结果上下文由 `reinitialize()` 绑定 |
| `SensitivityAnalyzer` | 数值容差 | 绑定 Objective、Registry、共享 FEA 控制器和试探模型更新入口，建立多工况总灵敏度 |
...
建立静态运行时关系和资源；`step()` 按当前 iteration 从 Params 的已提交定义创建新的
Assembly，完成求解、目标和灵敏度分析，再由 Updater 提交下一轮 Params 状态；
...
Controller._opt_loop()
    ...
    │   ├─ Solver.solve(fea_controller, jacobian_names)
    │   ├─ ObjectiveFunction.reinitialize(iteration, fea_controller, results)
    │   ├─ ObjectiveFunction.build_evaluation()
    │   ├─ SensitivityAnalyzer.build_sensitivities()
    │   │   └─ DesignRegistry.update_assembly(design_vars)（仅建立灵敏度试探图）
