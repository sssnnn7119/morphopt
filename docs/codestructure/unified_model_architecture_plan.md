...
映射、迁移阶段和验收状态。迁移文档引用设计文档中的接口，设计文档不依赖迁移文档中的
临时名称。
...
- 构造属性统一通过同名 `property` 对外访问；运行时状态保留为私有字段，使用
  `build_*`、`get_*`、`export_*`、生命周期或持久化方法访问，不提供运行时 property。
- 每个类的设计说明固定按“构造属性、运行时属性、属性接口、外部接口方法、内部辅助函数”
  的顺序组织；没有内容的区块保留空表，并明确标注“空”或继承范围。
- 子类表格只列本类新增或重写的成员；未重写的继承成员不重复列出，空表保留并说明继承范围。
  已列出的重写接口通过“来源”列标注最上层父类或协议定义位置，本类新增接口使用 `-`。
- `get_*` 只读取已有结果且不触发重计算；`set_*` 修改已有属性或定义；`add_*` 向已有
...
- 注册表使用只读映射 property 作为唯一事实来源；单项访问统一使用
  `registry[name]`，不重复提供同义 `get_<item>()`。
